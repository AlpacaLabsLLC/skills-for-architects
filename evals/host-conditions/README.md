...
file writing. This is an imposed capability condition, not a claim about every
current desktop product. Actual R4 host acceptance identifies the product, version,
local adapter and granted file access separately.
...
built with the current fresh project model by `make-fixture.sh` and validated by
the canonical `csv-library.py`. Product names, specifications and prices are synthetic.
Historical August results remain dated evidence for their original source.

---
name: nyc-amendment-check
description: "Find evidenced amendments affecting a bounded NYC provision and time interval. Use for \"what amended this section\"; not for claim an exhaustive legal history or treat every bulletin as an amendment."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-amendment-check

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find evidenced amendments affecting a bounded NYC provision and time interval. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Base source/edition/provision and date interval.
- **Primary output:** Bounded amendment evidence and search limitations.
- **Source dependencies:** `source:nyc-local-laws`, `source:nyc-agency-rules`, `source:nyc-buildings-bulletins`.
- **Coverage:** N12; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Claim an exhaustive legal history or treat every bulletin as an amendment.
...
3. Read evidenced amends/supersedes relationships and retrieve the identified instruments within the requested interval. Separate proposed, enacted and effective status. Absence in the maintained map means no indexed amendment found, not proof the provision is unamended.

...
description: "Find an NYC Existing Building Code provision or its publication/effective-date evidence. Use for \"existing-building code text\"; not for assume an enacted future code already applies; decide conversion compliance."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-existing-building-code

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an NYC Existing Building Code provision or its publication/effective-date evidence. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Edition and section/topic; filing/work dates only for applicability.
- **Primary output:** Existing-building provision evidence.
- **Source dependencies:** `source:nyc-existing-building-code`.
- **Coverage:** N01; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Assume an enacted future code already applies; decide conversion compliance.

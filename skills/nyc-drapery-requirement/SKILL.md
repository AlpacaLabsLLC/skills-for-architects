---
name: nyc-drapery-requirement
description: "Find NYC requirements for curtains, drapery or decorative textiles. Use for \"curtain or drapery fire requirement\"; not for treat a curtain as an interior wall finish or substitute upholstery testing."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-drapery-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC requirements for curtains, drapery or decorative textiles. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Hanging/decorative textile application, occupancy and location if known.
- **Primary output:** Drapery requirement evidence.
- **Source dependencies:** `source:nyc-fire-code`, `source:nyc-building-code`.
- **Coverage:** N11; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Treat a curtain as an interior wall finish or substitute upholstery testing.

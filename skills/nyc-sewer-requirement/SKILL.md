---
name: nyc-sewer-requirement
description: "Find NYC sewer or wastewater connection requirements. Use for \"sewer connection rule\"; not for design drainage or approve connection capacity."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-sewer-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC sewer or wastewater connection requirements. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Sewer/wastewater activity and location context if material.
- **Primary output:** Sewer requirement evidence.
- **Source dependencies:** `source:nyc-sewer-connections`.
- **Coverage:** N07; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Design drainage or approve connection capacity.

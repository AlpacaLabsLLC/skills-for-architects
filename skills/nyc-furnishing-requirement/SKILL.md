---
name: nyc-furnishing-requirement
description: "Find NYC requirements for furnishings in an identified use context. Use for \"furniture or furnishing fire requirement\"; not for treat upholstery automatically as wall finish or approve a product."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-furnishing-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC requirements for furnishings in an identified use context. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Furnishing type, occupancy and use location if known.
- **Primary output:** Furnishing requirement evidence.
...
- **Not this operation:** Treat upholstery automatically as wall finish or approve a product.

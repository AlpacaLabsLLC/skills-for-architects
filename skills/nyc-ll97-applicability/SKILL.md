---
name: nyc-ll97-applicability
description: "Gather bounded Local Law 97 applicability evidence and unresolved screening facts. Use for \"does LL97 apply\"; not for certify exemption, calculate emissions or declare complete applicability."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-ll97-applicability

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Gather bounded Local Law 97 applicability evidence and unresolved screening facts. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Building identifiers, floor-area evidence and relevant reporting year if known.
- **Primary output:** LL97 applicability evidence with unresolved conditions.
- **Source dependencies:** `source:nyc-ll97`.
- **Coverage:** N04; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Certify exemption, calculate emissions or declare complete applicability.
...
3. Separate observed building facts from statutory inclusion/exemption/exception conditions and reporting dates. Show conditions supported, contradicted and unknown with sources. A list or tax-lot fact alone is not an authoritative applicability decision.

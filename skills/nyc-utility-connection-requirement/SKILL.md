---
name: nyc-utility-connection-requirement
description: "Find NYC DEP water-connection requirements. Use for \"DEP water connection requirement\"; not for claim coverage of private electric/gas/telecom utility requirements."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-utility-connection-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC DEP water-connection requirements. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Water-connection activity and location context if material.
- **Primary output:** Water-connection requirement evidence.
- **Source dependencies:** `source:nyc-water-connections`.
- **Coverage:** N10; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Claim coverage of private electric/gas/telecom utility requirements.

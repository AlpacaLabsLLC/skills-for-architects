---
name: nyc-air-emissions-requirement
description: "Find NYC air-emissions or air-permit requirements for identified equipment or activity. Use for \"air permit requirement\"; not for calculate dispersion or decide permit eligibility."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-air-emissions-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC air-emissions or air-permit requirements for identified equipment or activity. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Equipment/activity and provision if known.
- **Primary output:** Air-emissions requirement evidence.
- **Source dependencies:** `source:nyc-air-pollution`.
- **Coverage:** N07; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Calculate dispersion or decide permit eligibility.

---
name: nyc-sidewalk-requirement
description: "Find NYC sidewalk construction, maintenance or permit requirements. Use for \"sidewalk requirement\"; not for design the sidewalk or determine street-tree obligations."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-sidewalk-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC sidewalk construction, maintenance or permit requirements. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Work/activity and location context if material.
- **Primary output:** Sidewalk requirement evidence.
- **Source dependencies:** `source:nyc-sidewalks`.
- **Coverage:** N10; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Design the sidewalk or determine street-tree obligations.

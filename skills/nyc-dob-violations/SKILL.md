...
Before acting, read the [host contract](../../docs/host-harness-contract.md) and this component’s [declaration](host-contract.json) (`skill:nyc-dob-violations`). Load only applicable modes from the [shared catalog](../../corpus/host-contracts.json); declarations do not grant access or permission. Use the actual host’s [delivery route](../../docs/host-adapters.md).
...
Query DOB, ECB and active-violation routes from the [source catalog](../../corpus/sources/catalog.json) for building violation records. Retrieve original metadata/field definitions before building a query. Preserve violation identity, issue date, source-described class/status, disposition and recorded amounts. Retrieve linked original documents when a conclusion depends on their contents. Pagination, join keys, status meanings and identifier formats come from the publisher at task time.

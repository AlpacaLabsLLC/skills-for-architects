...
Before acting, read the [host contract](../../docs/host-harness-contract.md) and this component’s [declaration](host-contract.json) (`skill:epd-research`). Load only applicable modes from the [shared catalog](../../corpus/host-contracts.json); declarations do not grant access or permission. Use the actual host’s [delivery route](../../docs/host-adapters.md).
...
Preserve material/product requirements, location, performance, dates and requested breadth. Search the catalog's `epd-research` routes and relevant original manufacturer/program publications; route labels are not claims about database size, coverage, accounts or availability. Use actual authorized access without guessing credentials or claiming an API exists.

For each candidate retrieve the original declaration and verify exact product/variant, region/plant, unit, registration, operator, issue/expiry and relevant indicators. Use epd-parser when structured extraction is needed. Keep inaccessible declarations and ambiguous variants explicit. Do not treat a search snippet or marketing claim as an EPD.

Return candidates with original links, selection rationale and material gaps. No real-product examples or numeric benchmarks are bundled. An optional library save requires the user's requested persistence and an exact preview under the EPD schema; retrieval alone authorizes no record adoption.

#!/usr/bin/env python3
"""Validate and atomically mutate Arch Studio CSV libraries."""
...
def main() -> int:
    ...
    parser.add_argument("command", choices=("init", "status", "validate", "import", "append", "update", "preview", "recover"))
    ...
    parser.add_argument("--operation", choices=("init", "import", "append", "update"))
    parser.add_argument("--request-id")
    parser.add_argument("--expected-sha256")
    args = parser.parse_args()

    if args.kind == "product" and args.command not in ("status", "validate"):
        import importlib.util
        module_path = Path(__file__).resolve().parents[3] / "tools/workspace/product_library.py"
        spec = importlib.util.spec_from_file_location("product_library", module_path)
        transactions = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(transactions)
        try:
            return transactions.run(args)
        except ValueError as exc:
            raise LibraryError(str(exc)) from exc
    if args.command in ("preview", "recover") or args.request_id or args.expected_sha256 or args.operation:
        raise LibraryError("transaction options apply only to product libraries")

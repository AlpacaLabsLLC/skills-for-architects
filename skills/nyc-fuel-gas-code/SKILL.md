---
name: nyc-fuel-gas-code
description: "Find a provision in an identified NYC Fuel Gas Code edition. Use for \"fuel-gas code section\"; not for size gas piping or approve equipment."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-fuel-gas-code

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find a provision in an identified NYC Fuel Gas Code edition. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Edition and provision or fuel-gas topic.
- **Primary output:** Fuel-gas provision evidence.
- **Source dependencies:** `source:nyc-fuel-gas-code`.
- **Coverage:** N01; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Size gas piping or approve equipment.

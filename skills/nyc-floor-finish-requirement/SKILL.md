---
name: nyc-floor-finish-requirement
description: "Find NYC requirements for floor finishes including carpet. Use for \"carpet or floor finish requirement\"; not for apply wall/ceiling finish classification to flooring."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-floor-finish-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC requirements for floor finishes including carpet. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Floor application, occupancy and location if known.
- **Primary output:** Floor-finish requirement evidence.
...
- **Not this operation:** Apply wall/ceiling finish classification to flooring.

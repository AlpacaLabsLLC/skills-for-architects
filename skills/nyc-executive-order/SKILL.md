---
name: nyc-executive-order
description: "Find an identified NYC executive order. Use for \"NYC executive order number\"; not for treat an order as a permanent amendment without evidence."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-executive-order

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an identified NYC executive order. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Order number/year or unambiguous title.
- **Primary output:** Executive-order evidence and scope unknowns.
- **Source dependencies:** `source:nyc-executive-orders`.
- **Coverage:** N12; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Treat an order as a permanent amendment without evidence.

---
name: nyc-referenced-standard
description: "Identify the standard and edition incorporated by an identified NYC provision. Use for \"which ASTM NFPA UL ASHRAE edition is referenced\"; not for supply copyrighted standard text or assume the publisher newest edition is incorporated."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-referenced-standard

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Identify the standard and edition incorporated by an identified NYC provision. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Controlling code source/edition/provision and standard designation if known.
- **Primary output:** Referenced-standard metadata and authorized access route.
- **Source dependencies:** `source:nyc-building-code`, `source:nyc-fire-code`, `source:nyc-energy-code`.
- **Coverage:** N11; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Supply copyrighted standard text or assume the publisher newest edition is incorporated.
...
3. Read the controlling provision's evidenced incorporation reference, then the canonical shared-standard metadata. Record referenced edition separately from publisher's latest edition. Standard records authorize metadata lookup only unless rights explicitly permit more; never synthesize protected clauses.

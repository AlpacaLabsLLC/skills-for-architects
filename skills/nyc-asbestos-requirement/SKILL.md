---
name: nyc-asbestos-requirement
description: "Find NYC asbestos survey, filing or abatement requirements. Use for \"asbestos requirement\"; not for provide abatement instructions or conclude material is asbestos-free."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-asbestos-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC asbestos survey, filing or abatement requirements. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Activity and provision/topic; work scope when material.
- **Primary output:** Asbestos procedural requirement evidence.
- **Source dependencies:** `source:nyc-asbestos`.
- **Coverage:** N07; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Provide abatement instructions or conclude material is asbestos-free.

---
name: nyc-accessibility-requirement
description: "Find NYC accessibility provisions with federal ADA obligations kept distinct. Use for \"NYC accessibility requirement\"; not for treat NYC approval as ADA compliance or design a compliant route."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-accessibility-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC accessibility provisions with federal ADA obligations kept distinct. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Element/use, NYC edition if known and requested regime.
- **Primary output:** NYC accessibility provision evidence with separate federal pointer.
- **Source dependencies:** `source:nyc-building-code`, `source:us-ada-standards`.
- **Coverage:** N08; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Treat NYC approval as ADA compliance or design a compliant route.

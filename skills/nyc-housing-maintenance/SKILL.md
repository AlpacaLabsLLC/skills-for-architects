---
name: nyc-housing-maintenance
description: "Find a NYC Housing Maintenance Code provision. Use for \"housing maintenance requirement\"; not for search address-specific HPD violations."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-housing-maintenance

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find a NYC Housing Maintenance Code provision. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Section or housing-maintenance topic.
- **Primary output:** Housing Maintenance Code provision evidence.
- **Source dependencies:** `source:nyc-housing-maintenance-code`.
- **Coverage:** N06; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Search address-specific HPD violations.

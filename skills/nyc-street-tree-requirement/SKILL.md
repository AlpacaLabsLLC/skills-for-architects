---
name: nyc-street-tree-requirement
description: "Find NYC Parks street-tree planting or work requirements. Use for \"street-tree requirement\"; not for select species or approve tree removal."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-street-tree-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC Parks street-tree planting or work requirements. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Planting/work activity and location context if material.
- **Primary output:** Street-tree requirement evidence.
- **Source dependencies:** `source:nyc-street-trees`.
- **Coverage:** N10; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Select species or approve tree removal.

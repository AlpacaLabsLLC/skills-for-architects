...
description: "Find an NYC Electrical Code provision and its referenced-edition evidence. Use for \"electrical-code section\"; not for substitute an unamended NEC edition or design a circuit."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-electrical-code

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an NYC Electrical Code provision and its referenced-edition evidence. This is a read-only lookup, not professional approval.

## Contract

- **Input:** NYC edition and section or electrical topic.
- **Primary output:** Electrical provision evidence.
- **Source dependencies:** `source:nyc-electrical-code`.
- **Coverage:** N02; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Substitute an unamended NEC edition or design a circuit.

---
name: nyc-emissions-limit
description: "Look up a published NYC building-emissions limit for an identified reporting period and category. Use for \"LL97 emissions limit\"; not for calculate annual emissions, penalties or LL97 applicability."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-emissions-limit

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Look up a published NYC building-emissions limit for an identified reporting period and category. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Reporting period and legally identified category; unknowns stay unknown.
- **Primary output:** Emissions-limit citation, unit and period.
- **Source dependencies:** `source:nyc-ll97`.
- **Coverage:** N04; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Calculate annual emissions, penalties or LL97 applicability.

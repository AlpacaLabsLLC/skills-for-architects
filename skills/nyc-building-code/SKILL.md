---
name: nyc-building-code
description: "Find an identified NYC Building Code provision or narrowly named building requirement. Use for \"building-code section or provision\"; not for calculate occupant load; select a code year; approve a building."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-building-code

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an identified NYC Building Code provision or narrowly named building requirement. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Code edition and section/topic.
- **Primary output:** Building-code provision evidence.
- **Source dependencies:** `source:nyc-building-code`.
- **Coverage:** N01; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Calculate occupant load; select a code year; approve a building.

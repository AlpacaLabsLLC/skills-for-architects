---
name: nyc-buildings-bulletin
description: "Find an identified DOB Buildings Bulletin. Use for \"Buildings Bulletin number\"; not for treat a bulletin as code text or infer universal applicability."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-buildings-bulletin

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an identified DOB Buildings Bulletin. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Bulletin number/year or unambiguous subject.
- **Primary output:** Buildings Bulletin evidence and document classification.
- **Source dependencies:** `source:nyc-buildings-bulletins`.
- **Coverage:** N12; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Treat a bulletin as code text or infer universal applicability.

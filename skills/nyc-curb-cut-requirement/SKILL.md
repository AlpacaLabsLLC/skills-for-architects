---
name: nyc-curb-cut-requirement
description: "Find NYC curb-cut requirements and documented agency review routes. Use for \"curb-cut requirement\"; not for approve driveway access or calculate zoning capacity."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-curb-cut-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC curb-cut requirements and documented agency review routes. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Location/use and work type if material.
- **Primary output:** Curb-cut requirement evidence.
- **Source dependencies:** `source:nyc-curb-cuts`.
- **Coverage:** N10; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Approve driveway access or calculate zoning capacity.

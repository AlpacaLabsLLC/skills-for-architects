---
name: nyc-construction-administration
description: "Find NYC construction administrative provisions, including Title 28 references. Use for \"construction administrative provision\"; not for make a permit determination or replace DOB filing review."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-construction-administration

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC construction administrative provisions, including Title 28 references. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Title/chapter/section or bounded administrative topic.
- **Primary output:** Administrative provision evidence.
- **Source dependencies:** `source:nyc-construction-administration`.
- **Coverage:** N01; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Make a permit determination or replace DOB filing review.

---
name: nyc-material-requirement
description: "Find NYC building-material requirements for a defined application not owned by a finish or furnishing lookup. Use for \"building material code requirement\"; not for approve a product; swallow wall finish, carpet, furniture or drapery requests."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-material-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find NYC building-material requirements for a defined application not owned by a finish or furnishing lookup. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Material/application, occupancy and code regime if known.
- **Primary output:** Application-qualified material requirement evidence.
- **Source dependencies:** `source:nyc-building-code`, `source:nyc-fire-code`.
- **Coverage:** N11; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Approve a product; swallow wall finish, carpet, furniture or drapery requests.

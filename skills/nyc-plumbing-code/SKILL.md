...
description: "Find a provision in an identified NYC Plumbing Code edition. Use for \"plumbing-code section\"; not for size a plumbing system or calculate fixture counts."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-plumbing-code

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find a provision in an identified NYC Plumbing Code edition. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Edition and provision or plumbing topic.
- **Primary output:** Plumbing provision evidence.
- **Source dependencies:** `source:nyc-plumbing-code`.
- **Coverage:** N01; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Size a plumbing system or calculate fixture counts.

---
name: nyc-lpc-requirement
description: "Find an LPC rule or work-review requirement. Use for \"LPC rule or alteration requirement\"; not for determine landmark designation or promise approval."
allowed-tools:
  - Read
  - Bash
  - WebFetch
...
# /as:nyc-lpc-requirement

<!-- architecture-studio:harness-compatibility -->
> Harness note: use `/as:<skill>` on Claude Code and `$<skill>` on Codex. Resolve `<skill-root>` as the directory containing this loaded `SKILL.md` and `<plugin-root>` as the plugin root that contains `skills/`, and use equivalent native tools when host tool names differ.

Find an LPC rule or work-review requirement. This is a read-only lookup, not professional approval.

## Contract

- **Input:** Rule/work type; designation evidence if applicability requested.
- **Primary output:** LPC requirement evidence.
- **Source dependencies:** `source:nyc-lpc-rules`.
- **Coverage:** N09; NYC scope with explicitly linked state/federal dependencies.
- **Not this operation:** Determine landmark designation or promise approval.

...
export async function loadCatalog(base = root) {
  ...
  for (const relationship of catalog.sources.relationships) {
    if (!/^https:\/\//.test(relationship.evidence)) fail('evidence');
  }
  return catalog;
}
...
export function validateRelationships(catalog) {
  ...
  for (const a of authorities) if (a.jurisdiction !== null && !has(jurisdictions, a.jurisdiction)) fail('authority jurisdiction');
  for (const s of sources) if (!has(authorities, s.authority) || (s.jurisdiction !== null && !has(jurisdictions, s.jurisdiction)) || (s.adapter !== null && !has(integrations, s.adapter))) fail('source references');
...
async function checkSource(source, { timeoutMs = 5000, fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
  ...
  if (source.identity === null || source.adapter === null) return { ...result, reason: 'pointer-only' };

...
`ffe_outputs.py` is a dependency-free, local Python 3 data-preparation and package-verification helper. It does **not** render documents, fetch products, mutate item records, send files, or expose a remote filesystem. `check` additionally requires host-installed Poppler `pdfinfo` and `pdftotext`, plus `pypdf` in the active Python; missing tools produce a blocked result. No dependency is installed automatically.

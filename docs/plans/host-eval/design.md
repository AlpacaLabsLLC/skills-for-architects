...
Windows is excluded by decision. ChatGPT is excluded because it cannot install the plugin in either version: Architecture Studio is a Claude Code plugin, and ChatGPT accepts MCP connectors only. There is no error rate to compare there until an MCP server exists. Pasting a `SKILL.md` into ChatGPT measures something else and is recorded separately as a portability probe, never as an arm.

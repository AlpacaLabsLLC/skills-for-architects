...
test('all maintained manifests validate; unknown editions remain null', async () => {
  const c = await loadCatalog(); assert.ok(c.sources.sources.length >= 3);
  assert.equal(c.sources.sources[0].edition, null);
  assert.equal(c.standards.length, 0);
  assert.ok(c.sources.sources.some(s => s.jurisdiction === null));
...
test('shared standards referenced from two jurisdictions need no duplication', async () => {
  ...
  for (const from of ['jurisdiction:fixture', 'jurisdiction:us-ny-nyc']) c.sources.relationships.push({ from, type: 'references', to: 'source:astm-e84', edition: null, effective_date: null, evidence: 'fixture-only' });
  validateRelationships(c); assert.equal(c.sources.sources.filter(s => s.id === 'source:astm-e84').length, 1);
});
...
test('directory-only sources never trigger transport and retain unknown applicability', async () => {
  const catalog = await loadCatalog();
  const source = catalog.sources.sources.find(s => s.identity === null);
  let calls = 0;
  const [result] = await checkSources([source.id], {fetchImpl: async () => {calls++; throw new Error('must not fetch');}, now: () => new Date('2026-09-13T00:00:00Z')});
  assert.equal(calls, 0); assert.equal(result.reason, 'pointer-only');
  assert.equal(result.checked_at, '2026-09-13T00:00:00.000Z');

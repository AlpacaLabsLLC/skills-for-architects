...
# Active guidance follows current owners; historical release notes remain history.
python3 - <<'PY'
from pathlib import Path
readme = Path('README.md').read_text()
for target in ('docs/architecture.md', 'docs/host-harness-contract.md', 'docs/workspace-model.md'):
...
assert "Harness-native execution" in release
assert "External-source navigation" in release
assert "New document model" in release
assert "Fifteen skills" in release
assert "no skill step points at a `.py` or `.sh` file" in release
...
echo "✓ v1.5 release contract documents its current package identities, authority links and preserved release history"

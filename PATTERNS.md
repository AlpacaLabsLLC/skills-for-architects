...
Start with the [architecture and authority map](docs/architecture.md). Use it to find the owner of the policy needed for the task; do not load the whole repository.

## Skill, tool, source or record

- A **skill** owns one bounded outcome and the judgment/procedure needed to achieve it. A meaningful multistep procedure can invoke several owners with explicit handoffs; do not split useful procedures merely to satisfy a verb or line-count quota. "Workflow" is a distinct, defined term (see [terminology](rules/terminology.md#skill-workflow-and-tool)); do not use it as a synonym for "skill" or "procedure" here or elsewhere.
